...
import { COLORWAYS, useColorway } from './colorwayStore'
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button
                ...
                aria-label="Choose dark colorway"
              >
            ...
            <PopoverContent align="end" className="w-64 p-2">
              ...
              <div className="flex flex-col gap-0.5">
                {COLORWAYS.map((cw) => {
                  const selected = colorway === cw.id
                  return (
                    <button
                      key={cw.id}
                      onClick={() => {
                        setColorway(cw.id)
                        toast.success(`Switched to ${cw.label}`)
                      }}
                      className="flex w-full items-center gap-3 rounded-md px-2 py-2 text-left hover:bg-accent"
                    >
                      <span className="flex h-6 w-12 shrink-0 overflow-hidden rounded ring-1 ring-border">
                        {cw.swatches.map((hex, i) => (
                          <span
                            key={i}
                      ...
                      <span className="flex-1">
                        <span className="block text-sm font-medium">{cw.label}</span>
